...
import {getChangeCoins, getMissingCoins, setMissingCoins} from "../services/coins";
...
test('Should correctly set mising coins', () => {
    setMissingCoins("100, 50, 1")
    expect(getMissingCoins()).toStrictEqual([100, 50, 1]);
});
...
test('Should return false to sell product and return all the money if product does not exist', () => {
    //expect(1).toBe(1);
    expect(sellProduct("Nuka Cola", "100 50 20 20")).toStrictEqual({sell: false, change: "100 50 20 20", code: 2});
});

test('Should return false to sell product and return all the money if no product provided', () => {
    //expect(1).toBe(1);
    expect(sellProduct("", "100 50 20 20")).toStrictEqual({sell: false, change: "100 50 20 20", code: 1});
});

test('Should return false to sell product if user provided incorrect money string', () => {
    //expect(1).toBe(1);
    expect(sellProduct("Cola", "fsddeg",[50, 10, 2, 1])).toStrictEqual({sell: false, change: "fsddeg", code: 5});
});
