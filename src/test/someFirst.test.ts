...
test('Should sell the product if some of the coins are missing for change', () => {
    ...
    expect(sellProduct("Cola", "100 50 20 20")).toStrictEqual({sell: true, change: "20 20 5 5 5 1 1", code: 0});
});

test('Should return false to set product and return all the money if change cannot be produced', () => {
    //expect(1).toBe(1);
    setMissingCoins([50, 10, 2, 1]);
    expect(sellProduct("Cola", "100 50 20 20")).toStrictEqual({sell: false, change: "100 50 20 20", code: 4});
});
